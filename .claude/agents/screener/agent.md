...
複数テーマ・複数地域でスクリーニングする場合、各クエリは独立しているため **並列実行** する。

```
NG（逐次）: AI-JP → AI-US → Defense-JP → Healthcare-US（各90s × 4 = 360s）
OK（並列）: AI-JP | AI-US | Defense-JP | Healthcare-US（最長90s = ~90s）
```

並列実行する方法: 複数の Bash ツール呼び出しを1つのメッセージで同時に発行する。
