...
### 9. PFバランス目標の提示（2段階 × レンジ制）

判定結果に応じたPFの目標レンジを提示する。**レンジ内なら調整不要。**

| 判定 | インカム | グロース | ヘッジ | キャッシュ |
|:---|:---|:---|:---|:---|
| **normal** | 45-55% | 25-30% | 5% | 15-20% |
| **risk-off** | 50-55% | 10-20% | 5-10% | 25-35% |

risk-off 切替トリガー:
- リスク判定スコア -3 以下
- または VIX > 25
- または強制ルール発動（VIX>40, F&G<10, WTI>$110）
- 上記いずれにも該当しなければ normal

**JEPI の分類**: ハイブリッド。保有比率を 50:50 でインカム枠とグロース枠に分割カウント。
...
| ルール | 下限 |
|:---|:---|
| 保有地域数 | 最低3地域（例: 米国+日本+欧州 or ASEAN） |
| 単一地域 | 60%以下 |
| 第3地域 | 10%以上（欧州0%等の再発防止） |
