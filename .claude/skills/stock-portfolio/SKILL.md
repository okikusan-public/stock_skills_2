...
地域/セクター/通貨/規模のHHI（ハーフィンダール指数）を算出し、ポートフォリオの偏りを分析する。規模別構成（大型/中型/小型/不明）テーブルを含む4軸分析（KIK-438）。
...
- 各軸の構成比率（**規模別構成テーブル: 大型/中型/小型/不明**）
- リスクレベル判定

### health の出力項目
- 銘柄（**小型株は `[小型]` バッジ付き**） / 損益率 / トレンド（上昇/横ばい/下降） / **クロスイベント**（ゴールデンクロス/デッドクロス） / 変化の質（良好/1指標↓/複数悪化） / アラート / **長期適性** / **還元安定度**
