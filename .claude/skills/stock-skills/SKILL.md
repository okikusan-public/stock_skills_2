...
- **GraphRAG 接続時は必ず dual-write を実行する。ローカル保存のみで終わらせない**
- オーケストレーターはサブエージェントの結果を受け取った後、上記テーブルに従い merge_screen / merge_note / merge_report を実行する
...
```
✅ ローカル保存済み（data/notes/2026-04-20_NFLX_lesson.json）
✅ GraphRAG 同期済み
```
...
```
✅ ローカル保存済み（data/notes/2026-04-20_NFLX_lesson.json）
⚠️ GraphRAG 未同期（Neo4j 未接続）
→ 「sync して」で同期できます
```

### 3. データ同期（sync）

ユーザーから「sync して」「GraphRAG と同期」と指示された場合、data/ → GraphRAG の一方向同期を実行する。
