...
| エージェント | 保存関数 | data/ 保存 |
|:---|:---|:---|
| Screener | `tools/graphrag.py` merge_screen() | data/screening_results/{preset}_{date}.json |
| Analyst | `tools/graphrag.py` merge_report() | - |
| Researcher | `tools/graphrag.py` merge_note(type=observation) | - |
| Health Checker | `tools/graphrag.py` merge_note(type=observation) | data/session_logs/{date}.json |
| Strategist | `tools/graphrag.py` merge_note(type=review) | data/session_logs/{date}.json |
| Reviewer | `tools/graphrag.py` merge_note(type=review) | - |

売買が実行された場合は merge_trade() も実行する。
GraphRAG 未接続時は data/ への JSON 保存のみ実行する（graceful degradation）。
