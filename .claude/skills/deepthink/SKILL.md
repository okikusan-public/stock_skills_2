...
```
⚠️ MUST: Step 2b で GPT / Gemini / Grok の3つを必ず呼ぶ。省略不可。
...
📋 実行プラン:
  ...
  Step 2: 評価 + 3LLM並列レビュー（GPT反証 + Gemini整合性 + Grokセンチメント）
...
#### 2b. 3LLM並列レビュー（⚠️ 3つ全て必須。省略不可）

**オーケストレーターが GPT / Gemini / Grok を並列起動する。3つ全て呼ばなければ Step 2 は未完了。**

```
Agent(gpt-reviewer):    「この分析の穴は？反論は？見落としているリスクは？」
Agent(gemini-reviewer): 「lesson と矛盾していないか？別の解釈はあるか？」
Agent(grok-reviewer):   「この銘柄/テーマのXセンチメントは？市場のリアルタイム反応は？」
```

| LLM | 役割 | 呼び出し方法 |
|:---|:---|:---|
| GPT | 反証・リスク指摘 | `call_llm('gpt', 'gpt-5.4', prompt, reasoning='high')` |
| Gemini | lesson整合性・別解釈 | `call_llm('gemini', 'gemini-2.5-flash', prompt)` |
| Grok | Xセンチメント・市場反応 | `tools/grok.py` の `search_x_sentiment()` or `search_market()` |
